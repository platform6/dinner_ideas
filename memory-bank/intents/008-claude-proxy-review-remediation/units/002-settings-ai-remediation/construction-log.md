...
| 2026-09-01T17:35:00Z | 042 (post-deploy fix)       | fixed          | Checkpoint-4 smoke found model/limit `.upsert()` → `42501` on prod (column-only grants + `ON CONFLICT DO UPDATE`). Fix `ec41f22`: migration `20260901120000_ai_config_write_rpc.sql` (`set_ai_model_override` / `set_ai_daily_call_limit` security-definer RPCs) + `updateAiConfig` → `.rpc()`. pgTAP 256/256, vitest 180/180. DB pushed to prod; FE merged to main. Verified on prod: model 200, limit 200, Test connection OK. ADR-6. |
