...
First of three "Tier 2" intents; the existing shared login keeps working (public sign-up is `007-auth-flows`,
settings is `008-account-settings`).
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| 2026-08-28 | Pursue "Tier 2" full per-user accounts, split into 3 intents: `004-account-model`, `007-auth-flows`, `008-account-settings`                                               | Settings alone don't need the refactor, but the user wants real multi-user; splitting de-risks the migration before sign-up UI lands                         | Yes      |
...
| 2026-08-28 | `dinners_per_week` setting (ships in `008-account-settings`) will be household-level; container hangs off `households`                                                    | Consistent with the shared catalog/plan direction                                                                                                            | Yes      |
...
| 2026-08-28 | `household_invites` table + the "join existing household instead of creating one" trigger branch live in `004`; the invite-sending UI + email live in `007-auth-flows`    | Keeps all DB/provisioning logic in one intent and independently testable; `007-auth-flows` becomes UI-only                                                   | Yes      |
...
1. Begin Construction: Unit `001-household-data-model`, bolt `026-household-data-model`
2. `/specsmd-construction-agent --unit="001-household-data-model" --bolt-id="026-household-data-model"`
3. Then `027 → 028 → 029 → 030`; `031-account-model-ui` after `028`
...
- Blocks `007-auth-flows` and `008-account-settings`.
