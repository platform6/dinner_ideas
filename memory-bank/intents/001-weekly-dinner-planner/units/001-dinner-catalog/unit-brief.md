...
Owns the dinner/recipe domain: the schema for dinners and their ingredients, filterable tags (cuisine, kid-friendliness), access control, and the seed dataset the app launches with.

## Scope

### In Scope
- `dinners` schema: name, cuisine type, cook time, Rosie-approved flag, instructions, `is_active` flag (suppress/un-suppress)
...
|--------|-------------|------------|
| Dinner | A single recipe/meal option | name, cuisine_type, cook_time_minutes, rosie_approved, instructions, is_active |
...
| 003-dinner-step-by-step-instructions | Dinner step-by-step instructions | Must | Planned |
