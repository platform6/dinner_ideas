---
last_updated: 2026-09-08T23:15:00Z
total_decisions: 13
---
...
- **Status**: accepted
- **Date**: 2026-09-08
- **Bolt**: 067-plan-uniqueness-scope (plan-rollover-remediation)
- **Path**: `bolts/067-plan-uniqueness-scope/adr-011-scope-a-stale-invariant-rather-than-remove-it.md`
- **Summary**: A production outage — `23505 idx_weekly_plans_one_unlocked` on every attempt to pick a dinner — traced to a partial unique index reading "at most one draft plan per household, across all time". Written in bolt 027 when a plan had no week identity, so "one draft per household" and "one draft per household per week" described the same set of states; intent 011's planning-week rollover made them different, and the index kept enforcing the older, now-wrong one. **The invariant did not break — its scope went stale.** The instinct under outage pressure is to drop the index and let the app cope; decided instead to rescope it to `(household_id, start_date)`, preserving the real rule (one draft per week) while removing the accidental one. Uses `drop` + `create` rather than `create ... if not exists`, which would silently no-op against the existing index and ship nothing. Documents an **asymmetric rollback**: restoring the old index is impossible once a household holds drafts for two weeks.
- **Read when**: A unique constraint or index starts rejecting legitimate work after a model gains a new dimension (a week, a tenant, a version) — ask whether the rule is wrong or merely under-scoped, because dropping it loses a real guarantee that nothing else is enforcing. Also read before writing `create index if not exists` in a migration that intends to _change_ an existing index, and when a migration's rollback is not symmetric.

### ADR-12: Restate `SET search_path` Whenever a Function Is Replaced

- **Status**: accepted
- **Date**: 2026-09-08
- **Bolt**: 063-dinners-per-week-rule (dinners-per-week)
- **Path**: `bolts/063-dinners-per-week-rule/adr-012-restate-set-search-path-when-replacing-a-function.md`
- **Summary**: `create or replace function` **does not inherit** the existing function's `SET` clauses — omitting `set search_path = ''` in a replacement silently drops the hardening a prior migration added, with no error and no warning. Decided that every `create or replace` in this project restates its `SET` clauses in full, and that schema-qualification travels with it. The project's `advisor_hardening_test.sql` asserts `proconfig` on the hardened functions and does catch the omission, so the risk is bounded — an earlier draft of this ADR overstated it as untested and was corrected.
- **Read when**: Writing any `create or replace function` against a function an earlier migration hardened; adding a new `security definer` or `search_path`-pinned function (add it to `advisor_hardening_test.sql`, or that suite's guarantee silently covers one function fewer than it appears to).

### ADR-13: Write the Whole Dinner Aggregate in One Database Transaction

- **Status**: accepted
- **Date**: 2026-09-08
- **Bolt**: 060-recipe-save (recipe-manual-entry)
- **Path**: `bolts/060-recipe-save/adr-013-aggregate-write-in-one-transaction.md`
- **Summary**: Saving a recipe writes four tables (`dinners`, `dinner_ingredients`, `dinner_steps`, `dinner_tags`, plus find-or-create on `tags`), and PostgREST inserts are separate HTTP calls, so there is no transaction from the browser. The domain model established that seven of the Dinner aggregate's eight invariants are properties of a _complete_ Dinner — so a `dinners` row with no ingredients **is not an incomplete Dinner, it is not a Dinner** — making the aggregate boundary and the transaction boundary the same boundary. Decided on a **`security invoker`** Postgres function performing every insert in one transaction, called once over RPC. Rejected client-side compensation: story 005's stated reason (the compensating delete can itself fail) invites "so retry the delete", so the ADR records the stronger one — **the window is a visibility window, not only a failure window.** Between the `dinners` insert and the children, the row is committed and queryable: another member's catalog lists a dinner with no ingredients, it can be **picked for the week** (picking needs only an id), the shopping list aggregates nothing for it, and the cooking view shows its empty state — all on the path where nothing goes wrong at all. Compensation is cleanup for the failure case; this is the success case. Also rejected `security definer` despite a 19:1 project majority: those 19 bypass RLS deliberately, this one must not, and `invoker` satisfies story 005's "existing RLS policies unchanged, no new policy" by construction. Costs: validation deliberately duplicated in TypeScript and SQL for different audiences, a `jsonb` parameter for ingredients, and a function that must restate `set search_path = ''` (ADR-12) and be added to `advisor_hardening_test.sql`.
- **Read when**: Adding any write that spans more than one table and must be all-or-nothing — the reflex is to sequence the inserts client-side and clean up on failure, and this is why that is wrong _even when the cleanup works_. More generally, when weighing any compensating action: ask whether the intermediate state is merely **brief** or actually **invisible**, because other clients read committed rows and a short window is still a published one. Also read before changing `fn_create_dinner` to `security definer` for consistency with this schema's other functions — it is `invoker` on purpose, and that is what keeps the RLS insert policies in force. See also ADR-1, ADR-7, ADR-11, ADR-12.
